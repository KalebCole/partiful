...
class DeterministicMergeGateTests(unittest.TestCase):
    def test_rejects_all_required_predicate_failures(self) -> None:
        result = validate_gate({"head": "b" * 40, "reviewed_sha": "a" * 40, "paths": ["bad.go"], "allowed_paths": ["internal/app/**"], "blockers": [{"state": "OPEN"}], "checks": [{"state": "PENDING"}], "required_checks": ["test"], "no_required_ci": False, "latest_review": {"verdict": "REQUEST_CHANGES"}, "review_cycles": 4})
