...
def adopt(run=None)->str:
 command=["hermes","kanban","--board","partiful","create","implement: #34 application kernel","--assignee","partiful-implementer","--workspace",f"worktree:{ROOT}","--branch","partiful/issue-34","--tenant","partiful-wayfinder","--idempotency-key","partiful:implementation:34","--initial-status","running","--body",f"Adopt existing PR #49 directly in review phase. PR branch: partiful/issue-34-initial-implement. Exact SHA: {SHA}. Do not rerun implementation or create children. Review using docs/agentic-engineering.md and native same-card lifecycle. If review requests changes, check out and push that existing PR branch from this same card workspace; do not open a replacement PR.","--json"]
 if run is None:
