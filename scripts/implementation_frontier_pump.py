...
## Reviewer phase
After native request-review, checkout the exact PR head at the exact 40-character SHA (exact 40-character PR SHA) and prove detached HEAD equality. Run all nine categories: specification, correctness, domain_model, test_quality, edge_cases, security_privacy, maintainability, domain_adherence, evidence_rigor. Write the machine-parseable structured verdict to a file with `## Implementation review`, `Verdict: APPROVE|REQUEST_CHANGES`, `Commit: <40-sha>`, `Category-<name>: PASS|FAIL` (for example `Category-specification: PASS`), `RED:`, and `GREEN:`. Create the run-bound signed review with `python3 scripts/deterministic_merge_gate.py --issue {issue.number} --sign-review-file <unsigned-review-file> > <signed-review-file>`, then post that exact signed file to GitHub issue #{issue.number}. The merge gate rejects unsigned or modified comments. Count structural review events/structured reviews. On request changes, native-return this same card to `partiful-implementer` with evidence-block; after attempt 3 hard-block (max 3 reviews). On approval invoke `scripts/deterministic_merge_gate.py`. Escalate only contradictory requirements, safety choices, or genuinely unresolved behavior; architectural boundaries do not escalate.'''
