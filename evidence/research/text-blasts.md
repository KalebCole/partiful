...
The observed completion facts are therefore:
...
- no nested business field is inspected after that helper access.
...
also appends `notificationChannels` and optional `images`. The dated browser
interception recorded the nested `createTextBlast.message` transport shape.
The retained facts cover the `all-guests` `to` representation and the current
page-display flag.
...
retain or expose that private identifier. The evidence does not establish the
exception's semantics.
...
That helper disables send for exactly three observed reasons:
...
## Completion evidence limit

The current client does not inspect a business-success field or perform a
post-send read. The evidence therefore establishes submitted intent only. It
does not establish recipient status or delivery.

## Dated text-blast observation

A browser interception on 2026-03-24 observed `POST
https://api.partiful.com/createTextBlast`. The request used the callable
`data.params` envelope and nested `message` fields `text`, `to`,
`showOnEventPage`, and optional `images`. This did not establish delivery.
