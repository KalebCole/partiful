...
This confirms that current guest callables use the documented top-level
callable transport envelope.
...
Firebase callable protocol, the raw HTTP response is the documented callable
...
supports these narrow guest fields:
...
behavior, or whether guest records can omit fields outside this observed
narrow subset.
...
The same assets do not establish a non-empty element schema for
`phoneContactsToInvite` or `emailsToInvite`. Their member shapes remain
unknown.
...
client. It does **not** establish persisted invite state, delivery, or any
response property stronger than successful callable completion.
