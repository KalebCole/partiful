...
The Partiful Hermes Kanban board is the durable execution authority. Existing implementation cards keep the stable key `partiful:implementation:<issue>` and native dependencies control readiness. The dispatcher claims each ready card once; no repository-local scheduler recalculates the frontier.

## Single-card state machine

`implementing -> native request-review -> reviewing -> APPROVE -> deterministic gate -> merged`, or `reviewing -> REQUEST_CHANGES -> same-card implementing`. The implementation card body contains phase-specific instructions for both roles. `coding-worker` records Feature/Bug/Refactor RED then GREEN proof on public seams, exact paths, PR and handoff readback, and native-requests review from `code-reviewer`. The reviewer exact-SHA checks out the PR, evaluates all nine categories (specification, correctness, domain_model, test_quality, edge_cases, security_privacy, maintainability, domain_adherence, evidence_rigor), and posts a machine-parseable review naming the SHA and category verdicts. A request-change includes an evidence block and returns the same card to `coding-worker`; three structural reviews is a hard block. There are no child review or integrator cards.
...
Evidence is an explicit card mode assigned to `coding-worker`, not a separate profile. It permits only bounded credential-free public/repository investigation and a reviewed `unsupported` conclusion, and never credentials or live mutation. `coding-worker` and `code-reviewer` are audited fail-closed for exactly terminal/file/skills, empty `.env`, `env_passthrough: []`, `shell_init_files: []`, and `auto_source_bashrc: false`.

## Operator procedure

Create, prioritize, and link cards on the Partiful board. Native dependencies and the Hermes dispatcher own readiness and claims. The board never merges directly: merge is triggered only by same-card approval from `code-reviewer` through `scripts/deterministic_merge_gate.py`.
